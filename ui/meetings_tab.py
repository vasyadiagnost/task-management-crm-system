...
TRANSLATIONS = {
    "ru": {
        "btn_new": "➕ Новая встреча",
        "btn_edit": "✏️ Редактировать",
        "btn_delete": "🗑 Удалить",
        "btn_refresh": "🔄 Обновить",
        "btn_copy_summary": "📋 Скопировать сводку",
        "search": "Поиск",
        "search_placeholder": "Контакт / периодичность / заметки...",
        "status": "Статус",
        "all": "Все",
        "btn_reset": "Сбросить фильтры",
        "counter_overdue": "🟥 Просрочены: {count}",
        "counter_active": "🟦 Активны: {count}",
        "counter_done": "🟩 Проведены: {count}",
        "counter_week": "🟣 Ближайшие 9 дней: {count}",
        "col_id": "ID",
        "col_contact": "Контакт",
        "col_recurrence": "Периодичность",
        "col_deadline": "Дедлайн",
        "col_days_left": "До срока",
        "col_status": "Статус",
        "upcoming_title": "Встречи ближайшей недели",
        "note_title": "Смысл модуля",
        "note_text": "Здесь фиксируется не само календарное событие, а регулярность встречи и дедлайн её планирования.\n\nСекретари уже создают конкретное событие сами, когда приходит время.",
        ...
        "msg_select_meeting": "Выбери встречу",
        "msg_meeting_not_found": "Встреча не найдена",
        "msg_summary_copied": "Сообщение по встречам скопировано в буфер обмена",
        "title_message": "Сообщение",
        "title_error": "Ошибка",
        "meeting_new_title": "Новая встреча",
        "meeting_edit_title": "Редактирование встречи",
        "field_contact": "Контакт",
        "contact_placeholder": "Начните вводить ФИО...",
        "field_recurrence": "Периодичность встречи",
        "recurrence_placeholder": "Например: еженедельно / раз в месяц / раз в квартал",
        "field_deadline": "Дедлайн планирования",
        "btn_pick": "📅 Выбрать",
        "field_notes": "Заметки",
        "btn_save": "Сохранить",
        "btn_cancel": "Отмена",
        "date_pick_title": "Выбор даты",
        "msg_deadline_invalid": "Дедлайн планирования должен быть в формате дд.мм.гггг",
    },
    "en": {
        "btn_new": "➕ New meeting",
        "btn_edit": "✏️ Edit",
        ...
        "search": "Search",
        "search_placeholder": "Contact / recurrence / notes...",
        "status": "Status",
        "all": "All",
        "btn_reset": "Reset filters",
        "counter_overdue": "🟥 Overdue: {count}",
        "counter_active": "🟦 Active: {count}",
        "counter_done": "🟩 Completed: {count}",
        "counter_week": "🟣 Next 9 days: {count}",
        "col_id": "ID",
        "col_contact": "Contact",
        "col_recurrence": "Recurrence",
        "col_deadline": "Deadline",
        "col_days_left": "Time left",
        "col_status": "Status",
        "upcoming_title": "Meetings for the coming week",
        "note_title": "Module purpose",
        "note_text": "This module stores not the calendar event itself, but the meeting cadence and the planning deadline.\n\nSecretaries create the actual event themselves when the time comes.",
        ...
        "msg_select_meeting": "Select a meeting",
        "msg_meeting_not_found": "Meeting not found",
        "msg_summary_copied": "Meeting summary copied to clipboard",
        "title_message": "Message",
        "title_error": "Error",
        "meeting_new_title": "New meeting",
        "meeting_edit_title": "Edit meeting",
        "field_contact": "Contact",
        "contact_placeholder": "Start typing a full name...",
        "field_recurrence": "Meeting recurrence",
        "recurrence_placeholder": "For example: weekly / monthly / quarterly",
        "field_deadline": "Planning deadline",
        "btn_pick": "📅 Pick",
        "field_notes": "Notes",
        "btn_save": "Save",
        "btn_cancel": "Cancel",
        "date_pick_title": "Pick a date",
        "msg_deadline_invalid": "Planning deadline must be in dd.mm.yyyy format",
    },
}
...
STATUS_TRANSLATIONS = {
    ...
    "en": {
        ...
        MEETING_STATUS_PAUSED: "Paused",
        MEETING_STATUS_DONE: "Completed",
...
class LanguageMixin:
    ...
    def status_values_for_filter(self):
        return [self.tr("all")] + [self.display_status(v) for v in MEETING_STATUS_VALUES] + [self.display_status(MEETING_STATUS_OVERDUE)]


class MeetingsTab(ctk.CTkFrame, LanguageMixin):
    def __init__(self, parent, meeting_service=None):
    ...
    def _build_ui(self):
        ...
        ctk.CTkButton(top_frame, text=self.tr("btn_new"), width=160, height=34, corner_radius=10, command=self.open_add_window).pack(side="left", padx=5, pady=8)
        ctk.CTkButton(top_frame, text=self.tr("btn_edit"), width=140, height=34, corner_radius=10, command=self.open_edit_window).pack(side="left", padx=5, pady=8)
        ctk.CTkButton(top_frame, text=self.tr("btn_delete"), width=120, height=34, corner_radius=10, command=self.delete_selected).pack(side="left", padx=5, pady=8)
        ctk.CTkButton(top_frame, text=self.tr("btn_copy_summary"), width=170, height=34, corner_radius=10, command=self.copy_summary).pack(side="left", padx=5, pady=8)
        ctk.CTkButton(top_frame, text=self.tr("btn_refresh"), width=140, height=34, corner_radius=10, command=self.refresh).pack(side="right", padx=5, pady=8)
        ...
        self.status_filter = ctk.CTkComboBox(filter_frame, values=self.status_values_for_filter(), width=170, command=lambda _: self.refresh())
        self.status_filter.pack(side="left", padx=5, pady=10)
        self.status_filter.set(self.tr("all"))

        ctk.CTkButton(filter_frame, text=self.tr("btn_reset"), width=150, height=34, corner_radius=10, command=self.reset_filters).pack(side="right", padx=10, pady=10)
        ...
        self.active_label = ctk.CTkLabel(self.dashboard_frame, text=self.tr("counter_active", count=0))
        ...
        table_frame.grid(row=0, column=0, rowspan=2, sticky="nsew", padx=(0, 8))

        columns = ("id", "contact", "recurrence", "deadline", "days_left", "status")
        self.tree = ttk.Treeview(table_frame, columns=columns, show="headings")
        self.tree.heading("id", text=self.tr("col_id"))
        self.tree.heading("contact", text=self.tr("col_contact"))
        self.tree.heading("recurrence", text=self.tr("col_recurrence"))
        self.tree.heading("deadline", text=self.tr("col_deadline"))
        self.tree.heading("days_left", text=self.tr("col_days_left"))
        self.tree.heading("status", text=self.tr("col_status"))

        self.tree.column("id", width=60, anchor="center")
        self.tree.column("contact", width=220)
        self.tree.column("recurrence", width=170)
        ...
        self.tree.tag_configure("soon", background="#fff0cc")
        self.tree.tag_configure("paused", background="#e8e8e8")
        self.tree.tag_configure("active", background="#eeeeee")
        ...
        ctk.CTkLabel(upcoming_frame, text=self.tr("upcoming_title"), font=ctk.CTkFont(size=18, weight="normal")).pack(anchor="w", padx=12, pady=(10, 8))
        ...
        self.upcoming_tree.heading("contact", text=self.tr("col_contact"))
        self.upcoming_tree.heading("recurrence", text=self.tr("col_recurrence"))
        self.upcoming_tree.heading("deadline", text=self.tr("col_deadline"))
        self.upcoming_tree.heading("days_left", text=self.tr("col_days_left"))
        ...
        ctk.CTkLabel(note_frame, text=self.tr("note_title"), font=ctk.CTkFont(size=18, weight="normal")).pack(anchor="w", padx=12, pady=(10, 8))
        box = ctk.CTkTextbox(note_frame, height=120)
        box.pack(fill="both", expand=True, padx=8, pady=(0, 8))
        box.insert("1.0", self.tr("note_text"))
        box.configure(state="disabled")

    def _days_text(self, days_left):
        if days_left is None:
            return self.tr("dash")
        if days_left < 0:
            return self.tr("days_ago", count=abs(days_left))
        if days_left == 0:
            return self.tr("today")
        return self.tr("days_left", count=days_left)
    ...
    def refresh(self):
        ...
        self.status_filter.configure(values=self.status_values_for_filter())
        if self.status_filter.get() not in self.status_values_for_filter():
            self.status_filter.set(self.tr("all"))
        ...
        self.overdue_label.configure(text=self.tr("counter_overdue", count=counts.get(MEETING_STATUS_OVERDUE, 0)))
        self.active_label.configure(text=self.tr("counter_active", count=counts.get(MEETING_STATUS_ACTIVE, 0)))
        self.done_label.configure(text=self.tr("counter_done", count=counts.get(MEETING_STATUS_DONE, 0)))
        self.week_label.configure(text=self.tr("counter_week", count=len(upcoming_rows)))

        for row in rows:
            meeting = row["meeting"]
            contact_name = row["contact_name"] or self.tr("no_contact")
            recurrence = meeting.recurrence_rule or ""
            status = row["status"]
            days_left = row["days_left"]
            deadline = meeting.start_datetime.strftime("%d.%m.%Y") if meeting.start_datetime else ""
            days_text = self._days_text(days_left)
            ...
            self.tree.insert(
                "",
                "end",
                values=(meeting.id, contact_name, recurrence, deadline, days_text, self.display_status(status)),
                tags=(tag,),
            )
        ...
        for row in upcoming_rows[:12]:
            meeting = row["meeting"]
            deadline = meeting.start_datetime.strftime("%d.%m.%Y") if meeting.start_datetime else ""
            days_text = self._days_text(row["days_left"])

            self.upcoming_tree.insert(
                "",
                "end",
                values=(row["contact_name"] or self.tr("no_contact"), meeting.recurrence_rule or "", deadline, days_text),
            )
    ...
    def open_edit_window(self):
        if self.selected_meeting_id is None:
            self.show_message(self.tr("msg_select_meeting"))
            return
    ...
    def delete_selected(self):
        ...
        if not deleted:
            self.show_message(self.tr("msg_meeting_not_found"))
            return
        self.refresh()
    ...
    def copy_summary(self):
        ...
        self.show_message(self.tr("msg_summary_copied"))
...
class MeetingWindow(ctk.CTkToplevel, LanguageMixin):
    def __init__(self, parent_tab, mode="add", meeting_id=None):
        ...
        self.title(self.tr("meeting_new_title") if mode == "add" else self.tr("meeting_edit_title"))
    ...
    def build(self):
        ...
        self.repeat_label = ctk.CTkLabel(self, text=self.tr("field_recurrence"))
        self.repeat_label.pack(pady=(4, 2))
        self.repeat_entry = ctk.CTkEntry(self, width=form_width, placeholder_text=self.tr("recurrence_placeholder"))
        self.repeat_entry.pack(pady=(0, 8))

        self.deadline_label = ctk.CTkLabel(self, text=self.tr("field_deadline"))
        self.deadline_label.pack(pady=(4, 2))
        deadline_frame = ctk.CTkFrame(self, fg_color="transparent")
        deadline_frame.pack(pady=(0, 8))
        self.deadline_entry = ctk.CTkEntry(deadline_frame, width=460, placeholder_text="dd.mm.yyyy")
        self.deadline_entry.pack(side="left", padx=5)
        ctk.CTkButton(deadline_frame, text=self.tr("btn_pick"), width=120, height=34, corner_radius=10, command=lambda: self.open_calendar(self.deadline_entry)).pack(side="left", padx=5)

        self.status_label = ctk.CTkLabel(self, text=self.tr("status"))
        self.status_label.pack(pady=(4, 2))
        self.status_combo = ctk.CTkComboBox(self, values=[self.display_status(v) for v in MEETING_STATUS_VALUES], width=form_width)
        self.status_combo.pack(pady=(0, 8))
        self.status_combo.set(self.display_status(MEETING_STATUS_ACTIVE))

        self.notes_label = ctk.CTkLabel(self, text=self.tr("field_notes"))
        ...
        ctk.CTkButton(self, text=self.tr("btn_save"), command=self.save, width=220, height=34, corner_radius=10).pack(pady=(8, 12))
    ...
    def fill(self):
        ...
        self.status_combo.set(self.display_status(self.meeting.status or MEETING_STATUS_ACTIVE))
    ...
    def save(self):
        ...
        if planning_deadline == "INVALID":
            self.show_error(self.tr("msg_deadline_invalid"))
            return
        ...
        try:
            if self.mode == "edit":
                ...
                if not meeting:
                    self.show_error(self.tr("msg_meeting_not_found"))
                    return
            else:
